            ...
            scope.render = function() {
                ...
                var combinedSeriesSum = function (series) {
                    ...
                    for (k = 0; k < sum.length; k++) {
                        combinedSum.push({ 'sum': sum[k], 'scalesum': scaleSum[k] });
                    }

                    return combinedSum;
                };

                total.data = combinedSeriesSum(series);
                ...
                for (index=0; index<series.length; index++) {
                    ...
                    svg.append('g')
                        ...
                        .attr('x', function(d, i) {
                            var pos = 0;
                            for (var x=0; x<index; x++) {
                                if (xScale(series[x].data[i]) !==0 && xScale(series[x].data[i]) < minBarWithTextWidth ) {
                                    pos += minBarWithTextWidth;
                                } else {
                                    pos += xScale(series[x].data[i]);
                                }
                            }
                            return leftStartPosition + pos;
                        })
                        .attr('y', function(d, i) {
                    ...
                    if (scope.config.showLabel) {
                        svg.append('g')
                            ...
                            .attr('x', function(d, i) {
                                var textPos = leftStartPosition - barLabelPadding;
                                textPos += xScale(series[index].data[i]) !== 0 && (xScale(series[index].data[i]) < minBarWithTextWidth) ? minBarWithTextWidth : xScale(series[index].data[i]);
                                for (var x=0; x<index; x++) {
                                    textPos += xScale(series[x].data[i]) !== 0 && (xScale(series[x].data[i]) < minBarWithTextWidth) ? minBarWithTextWidth : xScale(series[x].data[i]);
                                }
                                return textPos;
                            })
                            .text(function(d) {
                ...
                if (scope.config.showTotal) {
                    svg.append('g')
                        ...
                        .attr('ry', border)
                        .attr('x', function(d) {
                            return leftStartPosition + totalPadding + d.scalesum;
                        })
                        .attr('y', function(d, i) {
                            return i * (barHeight + barPadding);
                        })
                    ...
                    svg.append('g')
                        ...
                        .attr('x', function(d) {
                            var textLength = d3.select(this).node().getComputedTextLength();
                            return leftStartPosition + totalPadding + d.scalesum + (totalBarWidth-textLength)/2;
                        });
                }
            };
        }
